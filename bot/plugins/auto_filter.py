...
async def auto_filter(bot: Client, update: Message):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("🎖️Go To Next 🎖️", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"✨ Check links At @CC_Linkz Found {(len_results)} Results For Your Query: <code>{query}</code>",
